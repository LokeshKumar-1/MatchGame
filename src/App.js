...
class App extends Component {
  ...
  componentDidMount() {
    this.timeIntervalId = setInterval(this.click, 1000)
    const randomImage = imagesList[0].imageUrl
    const randomId = imagesList[0].id
    this.setState({toMatchImg: randomImage, toMatchId: randomId})
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.score !== score) {
      ...
      const randomId = imagesList[index].id
      this.setState({toMatchImg: randomImage, toMatchId: randomId})
    }
    console.log('Update')
    return null
  ...
  filteringListByTabId = () => {
    const {initialTabId} = this.state
    const filteredList = imagesList.filter(item => {
      if (item.category === initialTabId) {
        return item
      }
      return null
    })
  ...
  playAgainBtnClicked = () => {
    this.setState({isGameEnded: false, score: 0, seconds: 60})
    this.timeIntervalId = setInterval(this.click, 1000)
  }

  render() {
    const {initialTabId, seconds, isGameEnded, score, toMatchImg} = this.state
    const filteredList = this.filteringListByTabId()
    console.log('render')
    return (
          ...
          <div className="content-container-width">
            {!isGameEnded ? (
              <>
                ...
                />
                <ul className="tabIdList">
                  {tabsList.map(item => (
                    <TabItem
                      item={item}
                      key={item.tabId}
                      isSelected={item.tabId === initialTabId}
                      changingTabId={this.changingTabId}
                    />
                  ))}
                </ul>
                <ul className="thumbnailImageList">
                  {filteredList.map(item => (
                    <ThumbnailItem
                      item={item}
                      key={item.id}
                      evaluatingImage={this.evaluatingImage}
                    />
                  ))}
                </ul>
              </>
              ...
              <div className="result-container">
                ...
                />
                <p className="result-statement">YOUR SCORE</p>
